...
* Each corresponding `<pair-name>` the portolio and parameter files must be named `<pair-name>_Input.csv` and `<pair-name>_Input_PortfolioParameters.yml`,
respectively. For example:
  * This pair of files is valid: `a_Input.csv` and
  `a_Input_PortfolioParameters.yml`
  * This pair of files is invalid: `a_Input.csv` and
  `b_Input_PortfolioParameters.yml`

* In the parameter file, `portfolio_name_in` and `investor_name_in` can be
whatever you like, it will populate the output files with columns
`portfolio_name` and `investor_name` with the values provided.

```{bash, comment="", echo = FALSE}
cat working_dir/10_Parameter_File/TestPortfolio_Input_PortfolioParameters.yml
```


```{r}
fs::dir_ls("working_dir/40_Results/")
```
